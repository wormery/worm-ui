...
import { getDOMCor, withDefaultsOfToRefs } from "../../utils/utils";
...
export default defineComponent({
    props: {
        str: String,
        curserStyleType: String,
    },
    setup(props) {
        const { str, curserStyleType } = withDefaultsOfToRefs(props, {
            str: "",
                ...
                setTimeout(() => {
                    if (cursorPosition.value < wordDOMs.value.length - 1) {
                        ...
                        if (isNumber(h) && h > maxh) {
                            maxh = h;
                            wordStyle.value.minHeight = maxh + "px";
                        }
                    ...
                    style: (() => {
                        ...
                        if (enableBlink.value) {
                            w.add
                                ...
                                .add.animationName(keyframes("blink", (a, w) => {
        ...
        return () => (<div ref={(el) => {
                typeWritingDOM.value = el;
            }} class="type-writing clean" style={[$.value.typeWritingStyle.style, $.value.clear]}>
        <div class="cursor" ref={(el) => (cursorDOM.value = el)} style={[$.value.typeWritingStyle.cursor.style]}></div>
        {words.value.map((word, index) => (<div class="word" style={[
