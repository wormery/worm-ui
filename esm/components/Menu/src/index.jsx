...
export default defineComponent({
    ...
    setup(props) {
        ...
        const routers = refProps.routers;
        //过度时长单位毫秒
        ...
        if (routers.value.length > 0) {
            active = ref({ item: routers.value[0], index: 0 });
        }
        else {
            active = ref({ item: {}, index: 0 });
        }
        //是否缩小侧边栏
        let isNarrow = ref(false);
        //用来检查当前线程范围内是否更新了"isNarrow"
        let oldIsNarrow = null;
        // 滑块的一些属性
        ...
        /** menu中的按钮单击事件 */
        const menuItemClick = function (item, index) {
        ...
        const updataSliderStyle = (function () {
             ...
             */
            const animationUpdataRec = function (i, el) {
                i -= 1;
                startUpdata(el);
                if (i > 0) {
            ...
            return function (active, isChangeNarrow) {
                var _a;
                if ((_a = active === null || active === void 0 ? void 0 : active.item) === null || _a === void 0 ? void 0 : _a.el) {
                    if (isChangeNarrow) {
        ...
        return () => (<div class={["container", isNarrow.value ? "narrow" : ""].join(" ")} style={[w.add.position("relative").add.height(vh(100)).out()]}>
        <div class="btn-background" style={[
                w.add
                    .position("absolute")
                    .add.right(px(10))
                    .add.top(PE(50))
                    ...
                    .out(),
                condStyleByRef(isNarrow, w.add.right(px(-15)).add.zIndex("-1").out()),
            ]}></div>
        <div class="btn" onClick={narrowClick} style={[
                w.add
                    .position("absolute")
                    .add.right(px(10))
                    .add.top(PE(50))
        ...
        class={[
          ...
          <div class="header" style={[
            ...
            <div class="logo" style={[
                condStyleByRef(isNarrow, w.shandbox(() => w.add.opacity("0").add.overflow("hidden").out())),
                transitionStyle,
            ]}>
              <span>WORM</span>
            </div>
          ...
          {routers.value.map((item, index) => (<div v-magic={"selection"} class={w.clean
                    .class("bar-item")
                    .add.backgroundColor(createHoverColor(w.inject(w.the.menu.backgroundColor)))
                    .pseudo(":hover")
                    .add.backgroundColor(createPressedColor(w.inject(w.the.menu.backgroundColor)))
                    .pseudo(":active")
                    .out()} ref={(el) => (item.el = el)} onClick={() => menuItemClick(item, index)} style={[
                    w.shandbox(() => w.add
                        .display("flex")
                        .add.alignItems("center")
                        .add.justifyContent("center")
                        ...
                        .add.fontSize(em(2))
                        .out()),
                    condStyleByRef(isNarrow, w.shandbox(() => w.add.margin(px(5), px(10)).add.padding("5px 5px").out())),
                    transitionStyle,
                ]}>
              {(() => {
                    const style = w.shandbox(() => w.add.display("flex").add.alignItems("center").out());
                    return (<>
                    <div class="icon" style={[
                            ...
                            w.shandbox(() => w.add
                                ...
                                .add.fontSize(em(1.3))
                                .out()),
                            condStyleByRef(isNarrow, w.shandbox(() => w.add.margin(px(0), px(3))).out()),
                        ]}>
                      <div 
                    // @ts-ignore
                    name={item.icon} style={[
                            w.shandbox(() => w.add
                                .display("flex")
                                .add.alignItems("center")
                                .add.justifyContent("center")
                    ...
                    <div class="title" style={[
                            w.shandbox(() => w.add
                                ...
                                .add.overflow("hidden")
                                .out()),
                            condStyleByRef(isNarrow, w.shandbox(() => w.add.width(px(0)).add.opacity("0").out())),
                            style,
                            transitionStyle,
                        ]}>
                      <span>{item.name}</span>
